...
courant du disque, pour les grilles de `cases/rescos-locales`.

Usage :
    python3 scripts/rescos-locales/check_no_loss.py BASE_REF [GRILLE_FILTRE]

Exemples :
    python3 scripts/rescos-locales/check_no_loss.py 7c77e3e
    python3 scripts/rescos-locales/check_no_loss.py 7c77e3e RESCOS-63
...
cloture, resume, expert, theorie, presentation, scenario, annexe-qr,
annexe-image, feuille-porte) sont extraits avant et apres, puis compares avec le
meme seuil de ressemblance que `report_redundancy.py`
(SequenceMatcher(...).ratio() > 0.72) : les deux scripts mesurent la meme chose,
cote disparition plutot que cote doublon restant.
...
Comme dans German et RESCOS, on lit les BLOCS et non deux ZONES (`peda_bounds`
et `dd_bounds`, la voie d'AMBOSS) : quatre blocs (`therapy`, `redflags`,
`cloture`, et l'`annexe-dd` de 122 grilles sur 123) vivent en amont de la zone
pedagogique, disperses au fil de la page, et aucune zone continue ne les englobe
sans avaler aussi les criteres notes.
