...
courant du disque, pour les grilles German.

Usage :
    python3 scripts/german/check_no_loss.py BASE_REF [GRILLE_FILTRE]

Exemples :
    python3 scripts/german/check_no_loss.py 9382830
    python3 scripts/german/check_no_loss.py 9382830 German-2_
...
resume, presentation, annexe-image) sont extraits avant et apres, puis compares
avec le meme seuil de ressemblance que `report_redundancy.py`
(SequenceMatcher(...).ratio() > 0.72) : les deux scripts mesurent la meme
chose, cote disparition plutot que cote doublon restant.
...
dd_bounds). Ici on lit les BLOCS eux-memes, parce que deux d'entre eux
(`therapy-section`, `redflags-section`) vivent a l'interieur d'un `criteria-row`
de la section notee, disperses au fil de la page, et qu'aucune zone continue ne
les englobe sans avaler aussi les criteres notes.
...
def changed_grid_names(base_ref):
    ...
    result = subprocess.run(
        ["git", "diff", "--name-only", base_ref, "--", "cases/german"],
