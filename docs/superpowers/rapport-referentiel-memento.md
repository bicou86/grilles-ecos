# Écart au référentiel officiel — mémentos

456 formes canoniques tirées des 9 grilles officielles trouvées.
8561 items non officiels, dont **7134 sans répondant** (83 %).

| Corpus | Items orphelins |
|---|---|
| azygos | 2919 |
| german | 2474 |
| rescos | 1113 |
| amboss | 628 |

## Les 60 libellés orphelins les plus fréquents

- `Présentation avec nom, fonction et tâche` — 57×
- `Anamnèse sociale` — 49×
- `Examen cardiovasculaire` — 45×
- `Motif principal` — 41×
- `Examen abdominal` — 40×
- `Habitudes et mode de vie` — 40×
- `Antécédents chirurgicaux` — 37×
- `Mesures d'hygiène` — 37×
- `Dimension temporelle` — 35×
- `Palpation` — 31×
- `Irradiation` — 30×
- `Médicaments actuels` — 29×
- `Noxes` — 29×
- `Diagnostic de travail` — 28×
- `Habitudes de vie` — 26×
- `Symptômes généraux` — 26×
- `Recherche de symptômes spécifiques` — 25×
- `Conseil et soutien` — 25×
- `Symptômes B` — 25×
- `Diagnostic principal` — 25×
- `Examens complémentaires urgents` — 24×
- `Examen neurologique` — 24×
- `Examens complémentaires de première intention` — 23×
- `Antécédents personnels` — 23×
- `Hospitalisations et antécédents chirurgicaux` — 23×
- `Examen des extrémités` — 22×
- `Situation sociale` — 22×
- `Filet de sécurité` — 22×
- `Communication avec la patiente` — 21×
- `Inspection générale` — 20×
- `Examen pulmonaire` — 19×
- `Examen de la tête, yeux, oreilles, nez et gorge` — 19×
- `Tabagisme` — 19×
- `Communication avec le patient` — 18×
- `Motif de consultation` — 18×
- `Prise en charge thérapeutique` — 18×
- `Toxiques` — 18×
- `Examen cutané` — 18×
- `Examen du cou` — 18×
- `Signes vitaux` — 17×
- `Examens d'imagerie` — 17×
- `Facteurs de risque cardiovasculaire` — 17×
- `⊖ Facteurs soulageants` — 17×
- `Question d’entrée` — 17×
- `Laboratoire` — 16×
- `Début / Durée` — 16×
- `Diagnostic principal évoqué` — 15×
- `Examen thoracique` — 15×
- `Antécédents médicaux personnels` — 15×
- `Caractère de la douleur` — 15×
- `Début / durée` — 15×
- `Examens complémentaires proposés` — 14×
- `Auscultation pulmonaire` — 14×
- `Auscultation cardiaque` — 14×
- `Examens diagnostiques` — 14×
- `Question initiale` — 14×
- `Paramètres vitaux` — 14×
- `Retentissement des symptômes` — 13×
- `Symptômes d'accompagnement` — 12×
- `Sensibilité` — 12×

## Part d'items sans répondant, par SSP prioritaire

Les 44 SSP de `docs/ecos-priorites-2026.yaml`, triées par part décroissante d'items non officiels sans répondant dans le référentiel. N'entrent dans les décomptes que les items des grilles non officielles rattachées à cette SSP.

| SSP prioritaire | Items | Sans répondant | Part |
|---|---|---|---|
| Annonce de Mauvaise Nouvelle (SPIKES) | 13 | 13 | 100 % |
| Cervicalgies | 53 | 50 | 94 % |
| HTA (Suivi & Crise Hypertensive) | 52 | 49 | 94 % |
| Dyspnée | 167 | 153 | 91 % |
| Syncope & Perte de Connaissance | 231 | 210 | 90 % |
| Dépendance & Addictions (Alcool, Tabac, Drogues) | 97 | 88 | 90 % |
| Troubles du Sommeil | 151 | 136 | 90 % |
| Parésie - AVC | 70 | 63 | 90 % |
| Palpitations | 106 | 94 | 88 % |
| Douleur du Membre Inférieur | 61 | 54 | 88 % |
| Diarrhée | 120 | 106 | 88 % |
| Dysurie | 143 | 126 | 88 % |
| Prurit | 24 | 21 | 87 % |
| Dysfonction Érectile | 31 | 27 | 87 % |
| Œil Rouge & Douleur Oculaire | 214 | 186 | 86 % |
| Vertiges | 38 | 33 | 86 % |
| Plaie & Ulcère Cutané | 84 | 72 | 85 % |
| Incontinence Urinaire | 39 | 33 | 84 % |
| Lombalgies | 308 | 259 | 84 % |
| Douleur du Genou | 134 | 112 | 83 % |
| Toux | 395 | 330 | 83 % |
| Ictère | 51 | 42 | 82 % |
| Chute & Évaluation Gériatrique | 157 | 129 | 82 % |
| Amaurose & Perte Brutale de Vision | 206 | 168 | 81 % |
| Constipation | 32 | 26 | 81 % |
| Douleur Thoracique | 341 | 275 | 80 % |
| Douleur Abdominale | 618 | 484 | 78 % |
| Hypoacousie | 112 | 87 | 77 % |
| Éruption Cutanée | 305 | 236 | 77 % |
| Fatigue | 298 | 228 | 76 % |
| Troubles Cognitifs & Démence | 25 | 19 | 76 % |
| Céphalée | 201 | 152 | 75 % |
| Capacité de Discernement & Décision Partagée | 21 | 15 | 71 % |
| Baisse d'Acuité Visuelle Progressive | 0 | 0 | — (aucun item) |
| Bilan de Santé & Dépistage | 0 | 0 | — (aucun item) |
| Contraception & Conseil Préconceptionnel | 0 | 0 | — (aucun item) |
| Déficit Sensitivo-Moteur | 0 | 0 | — (aucun item) |
| Fièvre & État Fébrile | 0 | 0 | — (aucun item) |
| Lésion Pigmentée & Tache Cutanée | 0 | 0 | — (aucun item) |
| Nausées & Vomissements | 0 | 0 | — (aucun item) |
| Suivi Chronique, Compliance & Sortie d'Hospitalisation | 0 | 0 | — (aucun item) |
| TCA (Troubles du Comportement Alimentaire) | 0 | 0 | — (aucun item) |
| Troubles Psychotiques & Schizophrénie | 0 | 0 | — (aucun item) |
| Troubles de l'Humeur | 0 | 0 | — (aucun item) |
